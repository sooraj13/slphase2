...
public class AdminController {
	...
	public void  addStudent(String studentName,String className,String subjectName) {
		...
		//Player p1 = new Player(18, "Virat", "INDIA", 31);
		//session.save(p1);
		//Player p2 = session.get(Player.class,18);
		//p2.setPlayerName("Virat Kohli");
		//p2.setAge(35);
		//Player p3 = session.get(Player.class, 18);
		//session.delete(p3);
		//Player p4 = new Player("Rohit", "INDIA", 34);
		//Player p5 = new Player("Rahul", "INDIA", 29);
		//session.save(p4);
		//session.save(p5);
		//HQL queries
		//Query query = session.createQuery("from Player where team=:name order by playerName");
		//query.setString("name", "INDIA");
		//List<Player> playerList = query.list();
		//System.out.println("Player List Details");
